...
reportTypeList = [];
public displayedColumns :any[] =  [ 'dttime','duration','location'];
    ...
    */
    ...
    getReportData(form){
      if(form){
        ...
        this.userService.storeDataToDb(this.reportAdd,'StoppageReport').subscribe((data:any)=>{
          if(data.length>0){
            
            data.forEach(report_data => {
              var latlng = new google.maps.LatLng(report_data.lat, report_data.lngt);
              var geocoder = new google.maps.Geocoder();
              geocoder.geocode({ 'latLng': latlng },  (results, status) =>{
                console.log(status);
                  if(status == google.maps.GeocoderStatus.OK) {
                    report_data.location = results[0].formatted_address;
                  }
              });
            });
