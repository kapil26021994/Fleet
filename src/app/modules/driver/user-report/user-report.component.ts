...
reportTypeList = [];
public displayedColumns :any[] =  [ 'dttime','speed', 'kms','status'];
    ...
    */
    ...
    getReportData(form){
      if(form){
        ...
        this.userService.storeDataToDb(this.reportAdd,'vehicleRawDetailReport').subscribe((data:any)=>{
          if(data.length>0){
    ...
    formSubscribe() {
      this.companyName.valueChanges.subscribe((positionValue) => {
        ...
        this.filteredValues['speed'] = positionValue;
    ...
    getFormsValue() {
      this.dataSource.filterPredicate = (data, filter: string): boolean => {
        ...
        const resultValue =
        isStatusAvailable && 
          (data.speed.toString().trim().toLowerCase().indexOf(searchString.speed != null ? searchString.speed.toLowerCase() : '') !== -1 ||
          data.kms.toString().trim().toLowerCase().indexOf(searchString.kms != null ? searchString.kms.toLowerCase() : '') !== -1 ||
