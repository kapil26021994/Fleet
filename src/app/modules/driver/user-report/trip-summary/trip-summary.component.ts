...
filteredValues =  {idleTime: '',kmsCovered:'',movement: '',startDate: '',endDate:'',totalTime:''};
reportTypeList = [];
public displayedColumns :any[] =  ['idleTime','kmsCovered','movement','startDate','endDate','totalTime'];
    ...
    updateDataViaKey(value,key):void{
      ...
      getReportData(form){
        if(form){
          ...
          this.userService.storeDataToDb(this.reportAdd,'TripSummaryLocationReport').subscribe((data:any)=>{
            if(data.length>0){
      ...
      formSubscribe() {
        this.companyName.valueChanges.subscribe((positionValue) => {
          this.filteredValues['idleTime'] = positionValue;
          this.filteredValues['kmsCovered'] = positionValue;
          this.filteredValues['movement'] = positionValue;
          this.filteredValues['startDate'] = positionValue;
          this.filteredValues['endDate'] = positionValue;
          this.filteredValues['totalTime'] = positionValue;
      ...
      getFormsValue() {
        this.dataSource.filterPredicate = (data, filter: string): boolean => {
          ...
          const resultValue =
            data.idleTime.toString().trim().toLowerCase().indexOf(searchString.idleTime != null ? searchString.idleTime.toLowerCase() : '') !== -1 ||
            data.kmsCovered.toString().trim().toLowerCase().indexOf(searchString.kmsCovered != null ? searchString.kmsCovered.toLowerCase() : '') !== -1 || 
            data.startDate.toString().trim().toLowerCase().indexOf(searchString.startDate != null ? searchString.startDate.toLowerCase() : '') !== -1 ||
            data.endDate.toString().trim().toLowerCase().indexOf(searchString.endDate != null ? searchString.endDate.toLowerCase() : '') !== -1 ||
            data.totalTime.toString().trim().toLowerCase().indexOf(searchString.totalTime != null ? searchString.totalTime.toLowerCase() : '') !== -1;
          return resultValue;
        };
        this.dataSource.filter = JSON.stringify(this.filteredValues);
      }
